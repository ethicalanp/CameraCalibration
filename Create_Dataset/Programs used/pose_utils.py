...
def create_mask_from_mesh(vertices_3d, faces, transform_mat, camera_matrix, width, height):
    if vertices_3d.shape[0] != 4 or vertices_3d.shape[1] < 3:
        print("[ERROR] Invalid vertices_3d shape:", vertices_3d.shape)
        return None
    if not faces:
        print("[ERROR] No faces provided")
        return None

    print(f"[DEBUG] transform_mat:\n{transform_mat}")
    projected_3d = transform_mat[:3, :] @ vertices_3d
    z_values = projected_3d[2, :]  # Extract all z-values

    if np.all(z_values <= 0):
        print("[ERROR] All mesh vertices behind camera — skipping.")
        return None

    print(f"[DEBUG] z_values range: min={np.min(z_values):.3f}, max={np.max(z_values):.3f}, mean={np.mean(z_values):.3f}")
    projected_2d = camera_matrix @ projected_3d
    z_values_safe = np.where(z_values > 1e-6, z_values, 1e-6)
    projected_2d[:2] /= z_values_safe
    projected = projected_2d[:2].T

    print(f"[DEBUG] Projected points range: x=[{np.min(projected[:, 0]):.3f}, {np.max(projected[:, 0]):.3f}], y=[{np.min(projected[:, 1]):.3f}, {np.max(projected[:, 1]):.3f}]")
    projected[:, 0] = np.clip(projected[:, 0], 0, width - 1)
    projected[:, 1] = np.clip(projected[:, 1], 0, height - 1)

    print(f"[DEBUG] Clipped projected points range: x=[{np.min(projected[:, 0]):.3f}, {np.max(projected[:, 0]):.3f}], y=[{np.min(projected[:, 1]):.3f}, {np.max(projected[:, 1]):.3f}]")
    ...
    for face in faces:
        if len(face) != 3:
            skipped_non_tri += 1
            continue
        # Correctly index z-values for the face's vertices
        face_z = projected_3d[2, face]  # Use face as indices into the z-row
        if np.any(face_z <= 0):
            skipped_z += 1
            continue
        pts = np.array([projected[i] for i in face], dtype=np.float32)
        if np.isnan(pts).any():
            skipped_nan += 1
            continue
        if np.any(pts < 0) or np.any(pts >= [width, height]):
            skipped_bounds += 1
            continue
        cv2.fillConvexPoly(mask, np.round(pts).astype(np.int32), 255)
        rendered_faces += 1

    print(f"[DEBUG] Rendered: {rendered_faces} faces | Skipped non-triangular: {skipped_non_tri} | Skipped z<=0: {skipped_z} | Skipped NaN: {skipped_nan} | Skipped bounds: {skipped_bounds}")
    return mask if rendered_faces > 0 else None
...
def detect_Charuco_pose_board(img, matrix_coefficients, distortion_coefficients, markerSize = 6, totalMarkers = 50, 
                            number_markers_x = 8, number_markers_y = 5, square_width = 0.034, aruco_width = 0.021):
